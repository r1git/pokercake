...
class JoueursController extends AppController
{
	...
	if($annee==1)
		...
		if($mois==0)
			$range='date >= '.$annee.'0101 AND date <= '.$annee.'1231';	
		else
			$range='date >= '.$annee.$mois.'01 AND date <= '.$annee.$mois.'31';
	}
	
	$games = $this->Party->findAll($range);
	$totalp = sizeof($games);
	...
	foreach($everybody as $joueur_id => $joueur) {
		if($joueur[$NBRPARTIES]==0 || ($all!=2 && $joueur[$NBRPARTIES]<$totalp*0.1))
			...
			if($player['Joueur']['compagnon']>0)
				$req=' OR joueur_id ='.$player['Joueur']['compagnon'].')';
			else
				$req=')';
			if($range!='')
				$req=$req.' AND '.$range;
		
			$everybody[$joueur_id][$ORGA]= $this->Party->findCount('(joueur_id ='.$joueur_id.$req);
	...
	$lastparty = $this->Party->findAll($range, null, 'date DESC', 1, 0);
